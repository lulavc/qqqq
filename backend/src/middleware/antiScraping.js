...
class AntiScrapingMiddleware {
  constructor() {
    this.clientProfiles = new Map(); // Cache for client profiles
  }
      ...
      try {
        ...
        if (!result.allow) {
          ...
          return res.status(429).json({
            error: 'Too many requests detected. Access temporarily restricted.',
            code: 'RATE_LIMITED',
  ...
  async analyzeRequest(requestData) {
    ...
    // Update client-side info (assuming these headers/cookies might be set by client-side JS)
    profile.cookiesEnabled = Object.keys(requestData.cookies).length > 0;
    profile.jsEnabled = requestData.headers['x-js-enabled'] === 'true'; // Example header
    ...
    if (suspiciousScore >= config.BAN_THRESHOLD) {
      ...
      // Log ban
      logger.warn(`IP ${ip} banned for scraping. Score: ${suspiciousScore.toFixed(2)}, Duration: ${banDuration / 1000}s`);
      ...
      logger.info(`Challenge ${challengeType} applied to IP ${ip}. Score: ${suspiciousScore.toFixed(2)}`);
    }
  ...
  async recordChallengeResult(ip, success) {
    try {
      ...
      if (!profile) {
        ...
        profile = await this.loadProfile(ip, ""); // userAgent might not be available
        if (!profile.requestTimes) {
             logger.warn(`Profile for IP ${ip} not found when recording challenge result.`);
             return false;
        }
      }
      ...
      if (success) {
        ...
        profile.suspiciousScore = Math.max(0, profile.suspiciousScore - config.CAPTCHA_SUCCESS_REDUCTION);
      } else {
  ...
  async updateMouseMovements(ip, count) {
    try {
      ...
      if (!profile) {
        profile = await this.loadProfile(ip, ""); // userAgent might not be available
         if (!profile.requestTimes) {
             logger.warn(`Profile for IP ${ip} not found when updating mouse movements.`);
             return false;
        }
      }

      profile.mouseMovements = (profile.mouseMovements || 0) + count;
      await this.saveProfile(profile);
      return true;
